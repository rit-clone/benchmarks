...
class LabRamPretrained(torch.nn.Module):
    """LabRam PRetrained"""
...
class NeuralTransformer(nn.Module):
    def __init__(self, EEG_size=1600, patch_size=200, in_chans=1, out_chans=8, num_classes=1000, embed_dim=200,
                 ...
                 use_mean_pooling=True, init_scale=0.001, pretrained=None, **kwargs):
        super().__init__()
        ...
        # look for pretrained model for fine-tuning
    ...
    def forward_features(self, x, input_chans=None, return_patch_tokens=False, return_all_tokens=False, **kwargs):
        x = rearrange(x, 'B (w T) C A -> B C (A w) T', T=200)
    ...
    def forward(self, x, input_chans=None, return_patch_tokens=False, return_all_tokens=False, **kwargs):
        ...
        if input_chans is None:
            input_chans = [0, 20, 29, 30, 31, 32, 33, 39, 40, 41, 42, 43, 44, 45, 51, 52, 53, 54, 55, 63, 64, 65, 75]
